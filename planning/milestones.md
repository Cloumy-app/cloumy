...
- [ ] A-1. `budget_level` CHECK 5종 확장(V22) + `RouteGenRequest` 값 검증 3종
  - 「초절약」·「특별하게」 선택 시 루트 생성이 500으로 죽는다. **먼저 재현하고 고칠 것**
  - 근본 원인은 3계층 중 가운데(Spring DTO)가 아무 검증도 안 하는 것 — 걸렀으면 500이 아니라 400
- [ ] A-2. `apple` 로그인 분기 제거 — 서명 미검증으로 위조 토큰 계정 탈취 가능.
  개발자 계정도 없고 프론트 버튼도 없어 **검증 구현이 아니라 제거**로 확정
- [ ] A-3. JWT 블랙리스트 Redis 장애 fail-open — Redis 하나로 앱 전면 장애가 나는 구조.
  `RateLimitFilter`의 기존 fail-open 패턴을 그대로 따른다
- [ ] A-4. 구글 로그인 실구현 — **조사 중 발견**. 현재 동작하는 로그인이 `__DEV__` 개발자 로그인뿐이라
  릴리즈 빌드로는 아무도 로그인을 못 한다. 백엔드는 이미 완성, 프론트만 붙이면 됨
