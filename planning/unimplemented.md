...
### (번호추가) 숙소 날짜가 여행 기간과 어긋난 데이터가 있다
- **관련 태스크**: 위 조사 중 발견 (2026-07-29)
- **증상**: 테스트 루트 `448c2939`(2026-07-29~07-31)의 숙소 `남산힐호텔`이 **체크인 07-14 / 체크아웃 07-16**으로 들어가 있다. 여행 기간과 2주 떨어져 있어, Day↔숙소 날짜범위 조인이 어차피 아무것도 못 찾는다.
- **확인 필요**: 테스트 데이터가 망가진 것인지, **숙소 입력 UI가 날짜를 잘못 넣는 것인지** 구분해야 한다. 후자면 위 챗봇 연동보다 급하다.
- **검증 쿼리**:
  ```sql
  SELECT r.title, r.start_date, r.end_date, a.name, a.check_in_date, a.check_out_date
  FROM routes r JOIN accommodations a ON a.route_id = r.id
  WHERE a.check_in_date > r.end_date OR a.check_out_date < r.start_date;
  ```
- **우선순위**: 확인 후 판단 (입력 UI 버그면 높음)
